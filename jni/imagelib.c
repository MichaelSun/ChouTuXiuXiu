...
void png_read_data_from_asset(png_structp pngPtr, png_bytep data,
		png_size_t length) {
	AAsset *asset = (AAsset *) png_get_io_ptr(pngPtr);
	if (!asset) {
		return;
	}
	int count = AAsset_read(asset, data, length);
	if (count != length) {
		LOGD("png_read_data_from_asset error happen...");
	}
}
...
uglyimage* read_png(const char *file_name) {
	...
	int interlace_type = png_get_interlace_type(in_png_ptr, in_info_ptr);
	int number_passes = png_set_interlace_handling(in_png_ptr);
...
uglyimage* read_png_from_asset(AAssetManager *mgr, const char *fileName) {
	...
	png_set_read_fn(in_png_ptr, (png_voidp) asset, png_read_data_from_asset);
	png_read_info(in_png_ptr, in_info_ptr);
...
char* read_string_from_asset(AAssetManager *mgr, const char *fileName) {
	...
	char *content = (char*) malloc((int) length + 1);
	if (AAsset_read(asset, (void*) content, (size_t) length) <= 0) {
		free(content);
		content = NULL;
	} else {
		content[length] = '\0';
...
uglyimage* read_image_with_compress_by_area(const char *file_name,
	...
	if (file_type == TYPE_IMAGE_JPEG) {
		...
		if (number_passes == 1) {
			...
			if (out_width == w) {
				// 指向原图中一行数据
				unsigned char *in_line_pointer = (unsigned char *) malloc(
						stride);
				for (i = 0; i < height; i++) {
					row_pointer[0] = in_line_pointer;
...
void compress_image_by_area(uglyimage *data, COMPRESS_METHOD compress_method,
	...
	// 指向最终输出的图片全部数据
	unsigned char *pixels = (unsigned char *) malloc(out_height * out_stride);
	unsigned char *src = data->pixels;
	if (!pixels) {
		LOGD("out of memory when read file...");
		return;
	}
	...
	if (out_width == w) {
		...
		for (i = 0; i < height; i++) {
			memcpy(in_line_pointer, &src[i * stride],
		...
		for (i = 0; i < out_height; i++) {
			...
			if (iY != h - 1) {
				if (current_line <= iY) {
					...
					memcpy(base_line_pointer,
							&src[(y + current_line) * stride + x * channels],
							area_stride);
					current_line++;

					memcpy(next_line_pointer,
							&src[(y + current_line) * stride + x * channels],
							area_stride);
					current_line++;
				} else if (current_line == (iY + 1)) {
					memcpy(base_line_pointer, next_line_pointer, area_stride);
					memcpy(next_line_pointer,
							&src[(y + current_line) * stride + x * channels],
							area_stride);
					current_line++;
			...
			case 3:
				for (j = 0; j < out_width; j++, out_line_pointer += channels) {
					...
					for (c = 0; c < channels; c++) {
						up_left = base_line_pointer[iX * channels + c];
						up_right = base_line_pointer[(iX + 1) * channels + c];
						down_left = next_line_pointer[iX * channels + c];
						down_right = next_line_pointer[(iX + 1) * channels + c];
